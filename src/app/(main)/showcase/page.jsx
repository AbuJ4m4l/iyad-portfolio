...
const ProjectHoverOverlay = ({ project, onClose }) => {
  ...
  useEffect(() => {
    ...
    if (beforeFile && afterFile) {
      setBeforeAfterMedia({
        before: `https://rastan.shop${beforeFile.url}`,
        after: `https://rastan.shop${afterFile.url}`,
  ...
  const handleMediaSelect = (file) => {
    ...
    setSelectedMedia({
      url: `https://rastan.shop${file.url}`,
          ...
          {folder.files.map((file, index) => {
            ...
            return (
              ...
              >
                {isVideo ? (
                  <video
                    src={`https://rastan.shop${file.url}`}
                    className={
                      isShorts
          ...
          >
            {projects.map((folder, index) => {
              ...
              const media = thumbnailFile
                ? {
                    type: "image",
                    url: `https://rastan.shop${thumbnailFile.url}`,
                    filename: thumbnailFile.filename,
                  }
                : videoFile
                ? {
                    type: "video",
                    url: `https://rastan.shop${videoFile.url}`,
          ...
          categories.map(async (category) => {
            try {
              const res = await fetch(
                `https://rastan.shop/api/uploads/${category.name}`
              );
